...
install build tools; note that the version of `gcc` is 4.4.7 for CentOS 6.x.
...
download DPDK and SPDK source archives and unpack
...
if compiled with a newer kernel, such as `3.10.x`, the old gcc of CentOS 6.x may complain
...
one stupid fix would be increasing `max-inline-insns-single` to a larger size; to do so, edit `$(DPDK_DIR)/lib/librte_eal/linuxapp/igb_uio/Makefile`
...
edit the `.config` file under `x86_64-native-linuxapp-gcc` to modify DPDK configuration; to re-compile, simply invoke `make`.
...
then write the device ID (in the format of `VVVV DDDD SVVV SDDD CCCC MMMM PPPP`) to the `new_id` node of the driver to be attached to
    ...
    echo $ven_dev_id | sudo tee /sys/bus/pci/drivers/$driver_name/new_id
...
to unregister the device and unbind from the driver
    ...
    echo $ven_dev_id | sudo tee /sys/bus/pci/devices/$bdf/driver/remove_id
